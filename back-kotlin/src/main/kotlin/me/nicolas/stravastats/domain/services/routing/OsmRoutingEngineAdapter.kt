...
class OsmRoutingEngineAdapter : RoutingEnginePort {
    ...
    override fun generateTargetLoops(request: RoutingEngineRequest): List<RouteRecommendation> {
        ...
        if (recommendations.isEmpty()) {
            ...
            if (snappedStart != null) {
                ...
                val snapOffsetM = osmHaversineDistanceMeters(
    ...
    private fun buildReturnWaypointVariants(
        ...
        val distanceKm = max(1.0, osmHaversineDistanceMeters(anchor.lat, anchor.lng, start.lat, start.lng) / 1000.0)
    ...
    private fun mergeRoutePreviews(outbound: List<List<Double>>, inbound: List<List<Double>>): List<List<Double>> {
        ...
        if (
            inboundStart.size >= 2 &&
            outboundEnd.size >= 2 &&
            osmHaversineDistanceMeters(inboundStart[0], inboundStart[1], outboundEnd[0], outboundEnd[1]) <= 20.0
        ) {
            inboundStartIndex = 1
        }
    ...
    private fun fetchNearestRoadTraceRoute(
        ...
        sampled.forEach { point ->
            ...
            if (
                snapped.isNotEmpty() &&
                osmHaversineDistanceMeters(snapped.last().lat, snapped.last().lng, snappedPoint.lat, snappedPoint.lng) < 35.0
            ) {
                return@forEach
            }
    ...
    private fun toRouteCandidateFromPreview(
        ...
        val startOffsetMeters = osmHaversineDistanceMeters(
            preview.first()[0],
            preview.first()[1],
            request.startPoint.lat,
            request.startPoint.lng,
