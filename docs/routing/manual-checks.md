...
These checks validate GPS Art route-generation behavior on real OSRM-backed responses. The GPS Art smoke matrix is now scriptable and can be wired into local/CI smoke runs when OSRM data is available.
...
| --- | --- | --- | --- |
| GPS Art smoke | `./scripts/smoke-strava-art.sh` | [strava-art-smoke](./checks/strava-art-smoke.md) | shape generation returns a route and GPX export works |
| Anti-retrace | Legacy target script retired | [anti-retrace](./checks/anti-retrace.md) | strict for classic explorer routes; diagnostic-only for GPS Art when retrace improves drawing fit |
